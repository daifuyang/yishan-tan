...
const { assertCanManageMenus } = await import("~/features/menus/menus.policy");

describe("menus.policy", () => {
  describe("assertCanManageMenus", () => {
    it("passes for system admin", async () => {
      ...
      await expect(assertCanManageMenus(ctx)).resolves.toBeUndefined();
    });
    ...
    it("throws forbidden for non-admin", async () => {
      ...
      await expect(assertCanManageMenus(ctx)).rejects.toThrow();
    });
  });
});
