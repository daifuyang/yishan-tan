...
const { assertCanManagePosts } = await import("~/features/posts/posts.policy");

describe("posts.policy", () => {
  describe("assertCanManagePosts", () => {
    it("passes for system admin", async () => {
      ...
      await expect(assertCanManagePosts(ctx)).resolves.toBeUndefined();
    });
    ...
    it("throws forbidden for non-admin", async () => {
      ...
      await expect(assertCanManagePosts(ctx)).rejects.toThrow();
    });
  });
});
