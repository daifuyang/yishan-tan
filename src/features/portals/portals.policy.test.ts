...
const { assertCanManagePortals } = await import("~/features/portals/portals.policy");

describe("portals.policy", () => {
  describe("assertCanManagePortals", () => {
    it("passes for system admin", async () => {
      ...
      await expect(assertCanManagePortals(ctx)).resolves.toBeUndefined();
    });
    ...
    it("throws forbidden for non-admin", async () => {
      ...
      await expect(assertCanManagePortals(ctx)).rejects.toThrow();
    });
  });
});
