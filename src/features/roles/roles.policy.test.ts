...
const { assertCanManageRoles } = await import("~/features/roles/roles.policy");
...
describe("roles.policy", () => {
  describe("assertCanManageRoles", () => {
    it("passes for system admin", async () => {
      ...
      await expect(assertCanManageRoles(ctx)).resolves.toBeUndefined();
    });
    ...
    it("throws forbidden for non-admin", async () => {
      ...
      await expect(assertCanManageRoles(ctx)).rejects.toThrow();
    });
  });
});
