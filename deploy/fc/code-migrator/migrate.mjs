process.on('uncaughtException', e => { console.error('[uncaught]', e?.stack ?? e); process.exit(1); }); process.on('unhandledRejection', e => { console.error('[unhandledRejection]', e?.stack ?? e); process.exit(1); });
...
var DEFAULT_DATABASE_PORT = "5432";
...
function getDatabaseUrl(env = process.env) {
  ...
  if (!user || !password || !host || !name) return void 0;
...
async function firstTimeSeed(client, input) {
...
async function main() {
  ...
  if (input.dryRun) {
    ...
    const probe = src_default(url, { max: 1 });
    await probe`SELECT 1`;
    await probe.end();
    console.log("[db-migrate] connection probe OK");
    return;
  }
  const client = src_default(url, { max: 1, idle_timeout: 5 });
